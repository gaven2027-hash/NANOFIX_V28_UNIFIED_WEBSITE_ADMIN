create table if not exists public.portal_registration_requests (
  ...
  approved_role text,
  requested_role_group text not null default 'customer',
  approved_role_group text,
...
create index if not exists portal_registration_requests_status_role_idx on public.portal_registration_requests(status, requested_role, created_at);
create index if not exists portal_registration_requests_role_group_idx on public.portal_registration_requests(status, requested_role_group, approved_role_group, created_at);
create index if not exists portal_registration_requests_email_idx on public.portal_registration_requests(lower(email));
