...
const checks = [
  ...
  check(middleware.includes('shouldForceAdminAppHost(pathname, request.nextUrl.searchParams)'), 'admin paths are forced to app domain on production hosts'),
  check(middleware.includes('"/adminb": "admin"') && middleware.includes('"/adminb/:path*"'), 'admin backup short alias exists'),
  check(middleware.includes('"/customer": "customer"') && middleware.includes('"/customer/:path*"'), 'customer short alias exists'),
  check(middleware.includes('"/customerlb": "customer"') && middleware.includes('"/customerlb/:path*"'), 'customer backup short alias exists'),
  check(middleware.includes('loginRoleForPath') && middleware.includes('loginUrl.searchParams.set("role", role)'), 'protected short links preserve correct login role'),
