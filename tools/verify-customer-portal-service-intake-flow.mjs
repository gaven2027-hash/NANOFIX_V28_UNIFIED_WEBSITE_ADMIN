...
for (const file of requiredFiles) assert(exists(file), `Missing customer portal unified service request file: ${file}`);
...
if (requiredFiles.every(exists)) {
  ...
  const adminCoreApi = read('app/api/admin/service-operations/route.ts');
  ...
  ], 'Unified service request origin migration');
  ...
  noCustomerDocumentMutation(customerApi, 'Customer portal unified service request API');
  ...
  has(adminCoreApi, ['request_origin', 'customer_portal_request_type', 'related_warranty_id', 'portal_attachment_urls', 'portal_customer_notes'], 'Service Operations core API');
  has(ready, ['service_requests', 'leads', 'customer_document_feedback'], '/api/ready unified queue table checks');
}
