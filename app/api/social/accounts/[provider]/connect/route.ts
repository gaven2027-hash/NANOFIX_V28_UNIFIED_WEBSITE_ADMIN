...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  if (!parsed.success) return NextResponse.json({ ok: false, error: "Invalid social account payload", details: parsed.error.flatten() }, { status: 400 });
  ...
  return NextResponse.json({ ok: result.ok, bridge_ready: result.bridge_ready, skipped: result.skipped, warnings: result.warnings, ids: result.ids, next_step: result.ok ? "Test connection, then sync social data." : "Check account bridge audit persistence." }, { status: result.ok ? 200 : 503 });
}
