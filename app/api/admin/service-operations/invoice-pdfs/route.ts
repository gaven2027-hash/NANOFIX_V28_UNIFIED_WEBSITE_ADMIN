...
function pdfEscape(value: unknown) {
  return String(value ?? '').replace(/[\\()]/g, (match) => `\\${match}`).slice(0, 160);
}
...
function buildInvoicePdf(invoice: Invoice, items: InvoiceItem[]) {
  ...
  items.slice(0, 18).forEach((item, index) => {
    lines.push(`${index + 1}. ${pdfEscape(item.description)}  Qty ${formatMoney(item.qty)} x ${formatMoney(item.unit_price)} = ${formatMoney(item.amount)}`);
  });
  lines.push('', 'This PDF is generated by NANOFIX Service Operations.', 'For questions, contact NANOFIX support.');

  const textOps = lines.map((line, index) => `BT /F1 10 Tf 50 ${780 - index * 18} Td (${pdfEscape(line)}) Tj ET`).join('\n');
  const objects = [
  ...
  try {
    ...
    const { data: doc, error: docError } = await supabase
      ...
      .insert({
        ...
        metadata_json: { invoice_no: invoice.invoice_no, total: invoice.total, item_count: items.length }
      })
    ...
    const { data: task, error: taskError } = await supabase.from('unified_tasks').insert({
      ...
      description: `Invoice PDF generated for invoice ${invoiceId}.`,
      priority: 'P2',
    ...
    if (visibleToCustomer && customerId) {
      await supabase.from('notification_outbox').insert({
        ...
        payload_json: { source: 'invoice_pdf_generator', invoice_id: invoiceId, storage_path: storagePath },
