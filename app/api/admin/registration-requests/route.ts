...
function roleFromGroup(group: string, requestedRole: unknown, actorRole?: AdminRole) {
  if (cleanText(requestedRole, 'customer') === 'customer') return 'customer';
  if (group === 'inspection_repair') return 'engineer';
  if (group === 'operations') return 'operations_admin';
  if (group === 'finance') return 'finance';
  if (group === 'total_management') return actorRole === 'super_admin' ? 'super_admin' : 'content_admin';
  return 'content_admin';
}
function normalizeRoleGroup(value: unknown, requestedRole: unknown) {
  if (cleanText(requestedRole, 'customer') === 'customer') return 'customer';
  const group = cleanText(value, 'management').toLowerCase();
  return roleGroups.includes(group) && group !== 'customer' ? group : 'management';
}

async function findProfile(supabase: ReturnType<typeof createSupabaseAdminClient>, requestRow: Payload) {
  if (!supabase) return null;
  if (validUuid(requestRow.profile_id)) { const { data } = await supabase.from('profiles').select(profileColumns).eq('profile_id', requestRow.profile_id).maybeSingle(); if (data) return data; }
  if (validUuid(requestRow.auth_user_id)) { const { data } = await supabase.from('profiles').select(profileColumns).eq('auth_user_id', requestRow.auth_user_id).maybeSingle(); if (data) return data; }
  const email = cleanEmail(requestRow.email);
  if (email) { const { data } = await supabase.from('profiles').select(profileColumns).eq('email', email).order('created_at', { ascending: false }).limit(1).maybeSingle(); if (data) return data; }
  return null;
}
...
export async function GET(request: Request) {
  ...
  const roleGroup = url.searchParams.get('role_group');
...
export async function PATCH(request: Request) {
  ...
  const approvedRoleGroup = normalizeRoleGroup(body.approved_role_group || body.role_group || before.requested_role_group, before.requested_role);
  const approvedRole = roleFromGroup(approvedRoleGroup, before.requested_role, context?.role);

  if (action === 'reject') {
    const { data: rejected, error } = await supabase.from('portal_registration_requests').update({ status: 'rejected', approved_role_group: approvedRoleGroup, reviewer_notes: reviewerNotes, reviewed_by: context?.actorId, reviewed_at: now, updated_at: now }).eq('registration_request_id', id).select(requestColumns).single();
    if (error) return jsonError(error.message, 500);
    if (profileBefore?.profile_id) await supabase.from('profiles').update({ is_active: false, profile_status: 'rejected', review_status: 'rejected', updated_at: now }).eq('profile_id', profileBefore.profile_id);
  ...
  if (profileBefore?.profile_id) {
    const { data, error } = await supabase.from('profiles').update(profilePayload).eq('profile_id', profileBefore.profile_id).select(profileColumns).single();
    if (error) return jsonError(error.message, 500);
    profileAfter = data as Payload;
  } else {
    const { data, error } = await supabase.from('profiles').insert({ auth_user_id: validUuid(before.auth_user_id) ? before.auth_user_id : null, email: before.email, ...profilePayload, password_status: 'set', email_verified: false, created_at: now }).select(profileColumns).single();
    if (error) return jsonError(error.message, 500);
    profileAfter = data as Payload;
  }
  ...
  await auditLog({ actor_id: context?.actorId, actor_role: context?.role, action: 'approve_portal_registration_request', object_type: 'portal_registration_request', object_id: id, before_data: { request: before, profile: profileBefore }, after_data: { request: approved, profile: profileAfter, approved_role_group: approvedRoleGroup } });
