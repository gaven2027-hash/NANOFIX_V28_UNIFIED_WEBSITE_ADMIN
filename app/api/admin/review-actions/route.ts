...
const leadColumns = 'lead_id,intake_id,customer_id,name,phone,email,address,source_platform,ai_extracted_data,binding_status,priority,urgency_score,status,owner_id,created_at,address_text,issue_type,message,updated_at';
...
function inferLeadName(message: Payload) {
  const channel = cleanText(message.channel, 'Social');
  const customer = cleanText(message.customer_id);
  if (customer) return `Customer ${customer.slice(0, 8)}`;
  return `${channel} Lead`;
}

function inferPriority(risk: unknown) {
  const value = String(risk || '').toLowerCase();
  if (/(critical|high)/.test(value)) return 'P1';
  if (/(medium|warning)/.test(value)) return 'P2';
  return 'P3';
}

async function createLeadFromSocialMessage(supabase: ReturnType<typeof createSupabaseAdminClient>, message: Payload, actorId?: string) {
  if (!supabase) throw new Error('Supabase server client is not configured.');
  if (validUuid(message.lead_id)) return { lead: null, leadId: String(message.lead_id), created: false };

  const leadPayload = {
    customer_id: validUuid(message.customer_id) ? String(message.customer_id) : null,
    name: inferLeadName(message),
    phone: null,
    email: null,
    address: null,
    address_text: null,
    source_platform: cleanText(message.channel, 'social'),
    ai_extracted_data: {
      ...
      created_from: 'admin_review_actions'
    },
    binding_status: validUuid(message.customer_id) ? 'linked' : 'pending',
    priority: inferPriority(message.risk_level),
    urgency_score: String(message.risk_level || '').toLowerCase().includes('critical') ? 90 : String(message.risk_level || '').toLowerCase().includes('high') ? 75 : 40,
    status: 'new',
    owner_id: validUuid(actorId) ? actorId : null,
    issue_type: 'Social / Channel Message',
    message: cleanText(message.body)
  };

  const { data, error } = await supabase.from('leads').insert(leadPayload).select(leadColumns).single();
  if (error) throw new Error(error.message);
  return { lead: data, leadId: data.lead_id as string, created: true };
}
...
export async function PATCH(request: Request) {
  ...
  if (action === 'update_social_message') {
    ...
    let leadConversionStatus = conversionStatuses.includes(String(body.lead_conversion_status)) ? String(body.lead_conversion_status) : 'not_converted';
    const { data: before } = await supabase.from('social_messages').select(socialColumns).eq('message_id', messageId).maybeSingle();
    if (!before) return jsonError('Social message not found.', 404);
    ...
    let createdLead: Payload | null = null;

    if (handlingStatus === 'converted_to_lead') {
      try {
        const leadResult = await createLeadFromSocialMessage(supabase, before as Payload, context?.actorId);
        patch.lead_id = leadResult.leadId;
        patch.lead_conversion_status = 'converted';
        leadConversionStatus = 'converted';
        createdLead = leadResult.lead as Payload | null;
      } catch (error) {
    ...
    if (error) return jsonError(error.message, 500);
    await auditLog({ actor_id: context?.actorId, actor_role: context?.role, action: 'update_social_message', object_type: 'social_message', object_id: messageId, before_data: before ?? {}, after_data: { ...data, created_lead: createdLead, lead_conversion_status: leadConversionStatus } });
    if (createdLead?.lead_id) {
      await auditLog({ actor_id: context?.actorId, actor_role: context?.role, action: 'create_lead_from_social_message', object_type: 'lead', object_id: String(createdLead.lead_id), after_data: { lead: createdLead, source_message_id: messageId } });
    }
    return NextResponse.json({ ok: true, record: data, lead: createdLead });
  }
