...
async function loadPeople(supabase: NonNullable<ReturnType<typeof createSupabaseAdminClient>>) {
  ...
  return (data || []).map((p: Payload) => ({ actor_id: p.auth_user_id, role: p.role, label: p.display_name || p.email || p.auth_user_id, email: p.email || null, group_label: `${p.role} option / ${p.role} 选项` }));
}
