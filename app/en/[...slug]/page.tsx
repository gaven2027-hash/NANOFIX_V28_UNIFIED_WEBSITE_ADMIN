...
export async function generateMetadata({ params }: SlugPageProps): Promise<Metadata> {
  ...
  return buildPageMetadata(getRouteDefinition(routePath), "en");
}

export default async function EnglishSlugPage({ params }: SlugPageProps) {
  const resolvedParams = await params;
  const routePath = pathFromSlug(resolvedParams.slug);
  if (!hasRouteDefinition(routePath)) notFound();
  return <LegacyWebsitePage routePath={routePath} locale="en" homeHref="/en" />;
}
