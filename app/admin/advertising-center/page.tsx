...
const quickLinks = [
  ...
  { href: '/admin/advertising-center/creatives', title: 'Creatives & Copy / 素材文案', note: 'Create editable ad image, video, headline, CTA and landing page copy drafts.' },
  { href: '/admin/advertising-center/budgets', title: 'Budgets & Strategy / 预算策略', note: 'Submit budget changes, finance review and Super Admin final approval.' }
];

export default function Page() {
  return <div className="space-y-5"><section className="grid gap-3 xl:grid-cols-5">{quickLinks.map((item) => <Link key={item.href} href={item.href} className="rounded-3xl bg-white p-4 shadow-soft ring-1 ring-slate-200 transition hover:-translate-y-0.5 hover:ring-activeBlue"><div className="text-sm font-black text-slate-950">{item.title}</div><p className="mt-1 text-xs font-bold leading-5 text-slate-500">{item.note}</p></Link>)}</section><AdvertisingCenterWorkspace /></div>;
}
