...
import { PhoneCountryCodeInput, composePhoneNumber } from '@/components/PhoneCountryCodeInput';
...
function RegisterFormInner({ forcedContext }: { forcedContext?: RegisterContext }) {
  ...
  const cleanPhone = composePhoneNumber(phoneCountryCode, phoneLocal);
  ...
  async function submitRegistrationRequest(authUserId?: string) {
    await fetch('/api/public/registration-requests', {
      ...
      body: JSON.stringify({
        ...
        phone_local_number: phoneLocal.trim(),
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage(isAdmin ? 'Application submitted. Username / phone / email identifier is accepted for review. Super Admin will verify and assign the final role group. / 申请已提交。用户名、电话或邮箱任一识别资料均可进入审核，总管理员会核对并分配最终角色。' : 'Registration submitted. Phone is recorded as the required customer contact. Please verify your email or OTP if required, then sign in to Customer Portal. / 注册已提交，电话已作为客户必填联系方式记录。如系统要求，请先验证邮箱或 OTP，然后登录客户门户。');
  ...
  return (
    <>
      <div className="mt-5 text-center"><div className="mx-auto inline-flex rounded-full bg-blue-50 px-4 py-2 text-[11px] font-black uppercase tracking-[0.16em] text-activeBlue ring-1 ring-blue-100">{isAdmin ? 'Internal Admin App Registration / 内部管理系统注册' : 'Customer Portal Registration / 客户门户注册'}</div><h1 className="mt-4 text-2xl font-black tracking-tight text-slate-950">{isAdmin ? 'Request Internal Management Access' : 'Create Customer Account'}</h1><p className="mt-1 text-base font-black text-activeBlue">{isAdmin ? '申请内部管理系统权限' : '注册客户门户账号'}</p><p className="mt-4 text-sm font-bold leading-6 text-slate-700">{isAdmin ? 'Internal staff may register with any one of username, phone or email. No separate Engineer Portal/Register/Login.' : 'Customer registration requires phone / WhatsApp, then customers use the independent portal only.'}<br />{isAdmin ? '内部人员注册：用户名、电话、邮箱三项中任意填写一项即可提交；不再有独立工程师注册或登录。' : '客户会员注册电话号码为必填项，客户只进入独立客户门户。'}</p></div>
      <form className="mt-6 space-y-4" onSubmit={onSubmit}>
        {isAdmin ? <label className="block"><span className="mb-1 block text-xs font-black uppercase tracking-[0.12em] text-slate-500">Role Group / 角色分组</span><select className="w-full rounded-2xl border border-slate-200 bg-adminBg px-4 py-3 text-sm outline-none focus:border-activeBlue" value={roleGroup} onChange={(event) => setRoleGroup(event.target.value as RoleGroup)}>{internalGroups.map((group) => <option key={group.value} value={group.value}>{group.label} — {group.note}</option>)}</select><span className="mt-1 block text-[11px] font-semibold text-slate-500">Super Admin may correct this during review. / 总管理员审核时可以修改此分组。</span></label> : null}
        ...
        <PhoneCountryCodeInput countryCode={phoneCountryCode} onCountryCodeChange={setPhoneCountryCode} value={phoneLocal} onChange={setPhoneLocal} placeholder={isAdmin ? 'Phone / WhatsApp / 手机或 WhatsApp（内部注册可选）' : 'Phone / WhatsApp / 手机或 WhatsApp（客户必填）'} required={!isAdmin} />
        ...
        <button className="w-full rounded-2xl bg-activeBlue px-4 py-3 font-bold text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60" type="submit" disabled={loading}>{loading ? 'Submitting... / 提交中...' : 'Create Account / 创建账号'}</button>
        <p className="text-center text-[11px] font-bold leading-5 text-slate-500">Already have an account? / 已有账号？ <Link href={loginHref} className="text-activeBlue hover:underline">Sign in here / 点击登录</Link></p>
      </form>
    </>
  );
}
