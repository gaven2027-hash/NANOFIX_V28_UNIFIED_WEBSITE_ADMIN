...
function statusTone(status: unknown): 'blue' | 'green' | 'amber' | 'red' | 'gray' | 'cyan' {
  ...
  if (s === 'published' || s === 'approved' || s === 'edited') return 'green';
  if (s === 'draft' || s === 'pending_review' || s === 'queued' || s === 'editing' || s === 'manual_upload') return 'amber';
  if (s === 'archived' || s === 'rejected' || s === 'failed') return 'red';
  return 'blue';
}
  ...
  return (
    <div className="rounded-3xl bg-white p-5 shadow-soft ring-1 ring-slate-200">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <label><span className={labelClass}>Route Path / 页面路径</span><input className={inputClass} value={String(form.route_path || '')} onChange={(event) => setForm((current) => ({ ...current, route_path: event.target.value }))} placeholder="/" /></label>
        <label><span className={labelClass}>Status / 状态</span><select className={inputClass} value={String(form.status || 'draft')} onChange={(event) => setForm((current) => ({ ...current, status: event.target.value }))}>{pageStatuses.map((status) => <option key={status} value={status}>{status}</option>)}</select></label>
        ...
        <label className="md:col-span-2"><span className={labelClass}>Description / 描述</span><textarea className={`${inputClass} min-h-24`} value={String(form.description || '')} onChange={(event) => setForm((current) => ({ ...current, description: event.target.value }))} /></label>
        <label className="md:col-span-2"><span className={labelClass}>Metadata JSON / SEO 与页面元数据</span><textarea className={`${inputClass} min-h-40 font-mono text-xs`} value={metadata} onChange={(event) => setMetadata(event.target.value)} /></label>
      </div>
...
function BlockEditor({ row, routePath, onSaved, onClose, section }: { row: Row | null; routePath: string; onSaved: () => void; onClose: () => void; section?: WebsiteSectionConfig | null }) {
  ...
  function setVisualProvider(providerKey: string) {
    ...
    setForm((current) => ({
      ...current,
      visual_editor_provider: provider.key,
      visual_editor_status: provider.key === 'manual_final_asset_upload' ? 'manual_upload' : current.visual_editor_status || 'draft'
    }));
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-3xl bg-white p-5 shadow-soft ring-1 ring-slate-200">
        <div className="mb-4 flex items-start justify-between gap-3"><div><div className="text-xs font-black uppercase tracking-[0.16em] text-activeBlue">{row ? 'Edit Block' : 'New Block'}</div><h3 className="mt-1 text-xl font-black text-slate-950">{row ? formatValue(row.block_key) : `Create ${section?.title || 'Content Block'}`}</h3></div><button type="button" onClick={onClose} className="rounded-xl bg-slate-100 px-3 py-2 text-xs font-black text-slate-600 hover:bg-slate-200">Close / 关闭</button></div>
        {message ? <div className="mb-4 rounded-2xl bg-blue-50 px-4 py-3 text-sm font-bold text-blue-800 ring-1 ring-blue-100">{message}</div> : null}
        <div className="grid gap-4 md:grid-cols-2">
          <label><span className={labelClass}>Route Path / 页面路径</span><input className={inputClass} value={String(form.route_path || '')} onChange={(event) => setForm((current) => ({ ...current, route_path: event.target.value }))} /></label>
          <label><span className={labelClass}>Locale / 语言</span><input className={inputClass} value={String(form.locale || 'en')} onChange={(event) => setForm((current) => ({ ...current, locale: event.target.value }))} /></label>
          <label><span className={labelClass}>Block Key / 区块键名</span><input className={inputClass} value={String(form.block_key || '')} onChange={(event) => setForm((current) => ({ ...current, block_key: event.target.value }))} /></label>
          <label><span className={labelClass}>Content Type / 内容类型</span><select className={inputClass} value={String(form.content_type || 'section')} onChange={(event) => setForm((current) => ({ ...current, content_type: event.target.value }))}>{contentTypes.map((type) => <option key={type} value={type}>{type}</option>)}</select></label>
          <label><span className={labelClass}>Status / 状态</span><select className={inputClass} value={String(form.status || 'draft')} onChange={(event) => setForm((current) => ({ ...current, status: event.target.value }))}>{blockStatuses.map((status) => <option key={status} value={status}>{status}</option>)}</select></label>
          <label><span className={labelClass}>AI Image/GIF Editor / AI图文/GIF编辑平台</span><select className={inputClass} value={String(form.visual_editor_provider || defaultVisualEditorProvider)} onChange={(event) => setVisualProvider(event.target.value)}>{visualEditorProviders.map((item) => <option key={item.key} value={item.key}>{item.display_label_zh}</option>)}</select></label>
          <div className="rounded-2xl bg-purple-50 p-3 ring-1 ring-purple-100 md:col-span-2"><div className="text-sm font-black text-purple-950">{provider.label_zh}</div><div className="mt-1 text-sm font-bold text-purple-800">{provider.short_note_zh}</div><div className="mt-1 text-xs font-semibold text-purple-700">{provider.description}</div><div className="mt-2 flex flex-wrap gap-2"><Badge tone="blue">Priority {provider.priority}</Badge><Badge tone="cyan">{provider.category}</Badge><Badge tone={provider.supports_worker ? 'green' : 'amber'}>{provider.supports_worker ? 'Worker automation / 可自动化' : 'Manual upload / 人工上传'}</Badge><Badge tone={provider.supports_gif ? 'green' : 'gray'}>{provider.supports_gif ? 'GIF OK' : 'Still image only'}</Badge></div></div>
          <label><span className={labelClass}>Visual Asset Type / 素材类型</span><select className={inputClass} value={String(form.visual_asset_type || 'image')} onChange={(event) => setForm((current) => ({ ...current, visual_asset_type: event.target.value }))}>{visualAssetTypes.map((type) => <option key={type} value={type}>{type}</option>)}</select></label>
          <label><span className={labelClass}>Visual Editor Status / 编辑状态</span><select className={inputClass} value={String(form.visual_editor_status || 'draft')} onChange={(event) => setForm((current) => ({ ...current, visual_editor_status: event.target.value }))}>{visualEditorStatuses.map((status) => <option key={status} value={status}>{status}</option>)}</select></label>
          <label className="md:col-span-2"><span className={labelClass}>Visual Prompt / AI 图文/GIF 指令</span><textarea className={`${inputClass} min-h-24`} value={String(form.visual_prompt || '')} onChange={(event) => setForm((current) => ({ ...current, visual_prompt: event.target.value }))} placeholder="Generate a Singapore-style no-hacking waterproofing hero image..." /></label>
          <label><span className={labelClass}>Visual Template ID / 模板 ID</span><input className={inputClass} value={String(form.visual_template_id || '')} onChange={(event) => setForm((current) => ({ ...current, visual_template_id: event.target.value }))} /></label>
          <label><span className={labelClass}>Visual Model / 模型</span><input className={inputClass} value={String(form.visual_model || '')} onChange={(event) => setForm((current) => ({ ...current, visual_model: event.target.value }))} /></label>
          <label><span className={labelClass}>Visual Output URL / 输出图片/GIF URL</span><input className={inputClass} value={String(form.visual_output_url || '')} onChange={(event) => setForm((current) => ({ ...current, visual_output_url: event.target.value }))} /></label>
          <label><span className={labelClass}>Storage Path / 存储路径</span><input className={inputClass} value={String(form.visual_output_storage_path || '')} onChange={(event) => setForm((current) => ({ ...current, visual_output_storage_path: event.target.value }))} /></label>
          <label><span className={labelClass}>Alt Text / SEO 图片说明</span><input className={inputClass} value={String(form.visual_alt_text || '')} onChange={(event) => setForm((current) => ({ ...current, visual_alt_text: event.target.value }))} /></label>
          <label><span className={labelClass}>Cost Estimate / 预计成本</span><input className={inputClass} type="number" min="0" step="0.01" value={String(form.visual_cost_estimate || '')} onChange={(event) => setForm((current) => ({ ...current, visual_cost_estimate: event.target.value }))} /></label>
          <label className="md:col-span-2"><span className={labelClass}>Content JSON / 内容 JSON</span><textarea className={`${inputClass} min-h-56 font-mono text-xs`} value={contentJson} onChange={(event) => setContentJson(event.target.value)} /></label>
          <label className="md:col-span-2"><span className={labelClass}>Same-Position Preview JSON / 同位置预览 JSON</span><textarea className={`${inputClass} min-h-32 font-mono text-xs`} value={previewJson} onChange={(event) => setPreviewJson(event.target.value)} /></label>
        </div>
        ...
        {provider.key === 'manual_final_asset_upload' ? <div className="mt-3 rounded-xl bg-amber-50 px-3 py-2 text-xs font-bold text-amber-800 ring-1 ring-amber-100">Manual Final Image/GIF Upload is for self-edited final assets. Upload the final image/GIF, preview it in the same website position, then approve before publishing. / 人工上传最终图片/GIF 适合自己编辑好后上传，必须先同位置预览再发布。</div> : null}
      </div>
      <WebsiteSamePositionPreviewPanel routePath={String(form.route_path || '/')} blockKey={String(form.block_key || 'main')} contentType={String(form.content_type || 'section')} contentJsonText={contentJson} visualAssetType={String(form.visual_asset_type || 'image')} visualOutputUrl={String(form.visual_output_url || '')} visualOutputStoragePath={String(form.visual_output_storage_path || '')} visualAltText={String(form.visual_alt_text || '')} providerLabel={provider.label_zh} providerNote={provider.short_note_zh} />
    </div>
  );
}
...
export function WebsiteManagementWorkspace({ section }: { section?: WebsiteSectionConfig | null }) {
  ...
  useEffect(() => {
    ...
    setSelectedPage(null); setSelectedBlock(null); setCreatingPage(false); setCreatingBlock(false);
  }, [section]);
