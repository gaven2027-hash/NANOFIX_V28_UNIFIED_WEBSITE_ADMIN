...
export function BackupCenter() {
  ...
  async function runBackup(mode: 'queue' | 'run_now' | 'restore_dry_run') {
    setLoading(true);
    setMessage('');
    setSignedUrl(null);
    try {
      ...
      setMessage(mode === 'run_now' ? 'Encrypted backup completed and audited. / 加密备份已完成并写入审计。' : mode === 'queue' ? 'Backup job queued and audited. / 备份任务已排队并写入审计。' : 'Restore dry run completed and audited. / 恢复演练已完成并写入审计。');
      await loadBackups();
    } catch (error) {
  ...
  return (
    <SectionCard title="Backup & Download Center / 模块备份与下载中心" subtitle="Live encrypted Supabase backup jobs, signed links, restore dry run and audit logs. / 真实加密备份任务、限时签名链接、恢复演练和审计日志。">
      <div id="backup-download-center" className="scroll-mt-32 space-y-5">
        <div className="grid gap-4 xl:grid-cols-[380px_minmax(0,1fr)]">
          <div className="rounded-3xl bg-slate-50 p-4 ring-1 ring-slate-200">
            ...
            <div className="mt-4 grid gap-2">
              ...
              <button type="button" disabled={loading} onClick={() => runBackup('queue')} className="rounded-2xl bg-slate-900 px-4 py-3 text-sm font-black text-white hover:bg-slate-700 disabled:opacity-60">Queue Backup Job / 排队备份任务</button>
            </div>
          ...
          <div className="overflow-hidden rounded-2xl border border-slate-200">
            <table className="min-w-[980px] w-full text-left text-sm">
              <thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="p-3">Backup ID</th><th className="p-3">Module</th><th className="p-3">Schedule</th><th className="p-3">Status</th><th className="p-3">Encrypted File</th><th className="p-3">Created</th></tr></thead>
              <tbody className="divide-y divide-slate-100">
                {rows.map((item) => <tr key={item.backup_id} className="bg-white hover:bg-blue-50/50"><td className="max-w-48 truncate p-3 font-mono text-xs font-bold text-slate-600">{item.backup_id}</td><td className="p-3 font-bold text-slate-800">{item.module}</td><td className="p-3 text-xs font-bold text-slate-600">{item.schedule_cron || '—'}</td><td className="p-3"><Badge tone={statusTone(item.status)}>{item.status || 'unknown'}</Badge></td><td className="max-w-56 truncate p-3 text-xs font-bold text-slate-600">{item.encrypted_file_path || '—'}</td><td className="p-3 text-xs font-bold text-slate-500">{item.created_at || '—'}</td></tr>)}
                {!rows.length ? <tr><td colSpan={6} className="p-6 text-center text-sm font-bold text-slate-500">{loading ? 'Loading backup jobs... / 正在加载备份任务...' : 'No backup jobs found. / 暂无备份任务。'}</td></tr> : null}
              </tbody>
            </table>
          </div>
        </div>
        ...
        {signedUrl ? <a href={signedUrl} className="inline-flex rounded-2xl bg-emerald-600 px-4 py-3 text-sm font-black text-white hover:bg-emerald-700" target="_blank" rel="noreferrer">Open signed encrypted backup link / 打开限时加密备份链接</a> : null}
        <div className="rounded-2xl bg-amber-50 p-4 text-sm font-medium text-amber-900 ring-1 ring-amber-100">Storage: {storage}. No plaintext passwords, API keys, service role keys or unmasked sensitive data may be exported. / 不得导出明文密码、API 密钥、service role key 或未脱敏敏感数据。</div>
      </div>
    </SectionCard>
  );
}
