...
const channelSnapshots = [
  { label: 'Website Organic', zh: '网站自然', leads: 9, jobs: 3, href: '/website-management#website-organic-leads' },
  { label: 'Website Paid', zh: '网站广告', leads: 11, jobs: 2, href: '/website-management#website-paid-landing-leads' },
  { label: 'Social Organic', zh: '社媒自然', leads: 8, jobs: 2, href: '/social-media#organic-social-leads' },
  { label: 'Social Paid Ads', zh: '社媒广告', leads: 6, jobs: 1, href: '/admin/advertising-center#paid-social-ads' },
  { label: 'WhatsApp Direct', zh: 'WhatsApp 直接咨询', leads: 4, jobs: 1, href: '/social-media#whatsapp-ai-reply' }
];
  ...
  return (
    <div className="space-y-6">
          ...
          <div id="urgent-action-queue" className="scroll-mt-32 overflow-hidden rounded-2xl border border-slate-200">
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                <tr>
                  <th className="p-3">Task / 任务</th>
                  <th className="p-3">Owner / 负责人</th>
                  <th className="p-3">Priority</th>
                  <th className="p-3">Due / 到期</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {urgentActions.map((item) => (
                  <tr key={item.id} className="bg-white hover:bg-blue-50/50">
                    <td className="p-3">
                      <Link href={item.href} className="font-black text-slate-800 hover:text-activeBlue hover:underline">{item.title}</Link>
                      <div className="text-xs font-semibold text-slate-500">{item.zh}</div>
                      <div className="mt-1 text-xs font-bold text-activeBlue">{item.action}</div>
                    </td>
              ...
              <Link key={alert.id} href={alert.href} className="block rounded-2xl border border-slate-200 bg-white p-4 transition hover:-translate-y-0.5 hover:ring-1 hover:ring-activeBlue">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs font-semibold text-slate-500">{alert.zh}</p>
                  </div>
                  <Badge tone={riskTone(alert.risk)}>{alert.risk}</Badge>
                </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
        <SectionCard title="Intake & Lead Summary / 入口与线索汇总" subtitle="Website repair, member registration, WhatsApp, social organic, paid ads and unknown-source leads are separated. / 区分网站报修、会员注册、WhatsApp、社媒自然、广告和未知来源。">
          <div id="intake-lead-summary" className="scroll-mt-32 grid gap-3 md:grid-cols-2">
            {channelSnapshots.map((channel) => (
              <Link key={channel.label} href={channel.href} className="rounded-2xl border border-slate-200 bg-white p-4 transition hover:bg-blue-50 hover:ring-1 hover:ring-activeBlue">
                <div className="text-sm font-black text-slate-900">{channel.label}</div>
                <div className="text-xs font-semibold text-slate-500">{channel.zh}</div>
                <div className="mt-3 grid grid-cols-2 gap-2 text-center">
                  <div className="rounded-xl bg-slate-50 p-3"><div className="text-2xl font-black text-slate-950">{channel.leads}</div><div className="text-xs font-bold text-slate-500">Leads</div></div>
                  <div className="rounded-xl bg-slate-50 p-3"><div className="text-2xl font-black text-slate-950">{channel.jobs}</div><div className="text-xs font-bold text-slate-500">Jobs</div></div>
                </div>
              </Link>
            ))}
          </div>
        ...
        <SectionCard title="Revenue & Finance Summary / 收入与财务摘要" subtitle="Finance summary highlights cash collection, overdue invoices and ad-spend impact, then links to finance operation pages. / 财务摘要突出收款、逾期发票和广告花费影响，并跳转对应页面。">
          <div id="revenue-finance-summary" className="scroll-mt-32 grid gap-3 md:grid-cols-2">
            {[
              ...
              <Link key={item.label} href={item.href} className="rounded-2xl border border-slate-200 bg-white p-4 transition hover:ring-1 hover:ring-activeBlue">
                <div className="flex items-start justify-between gap-2"><div><div className="text-sm font-black text-slate-900">{item.label}</div><div className="text-xs font-semibold text-slate-500">{item.zh}</div></div><Badge tone={item.tone}>Open</Badge></div>
        ...
        <div id="operations-summary" className="scroll-mt-32 flex flex-wrap gap-2">
          {serviceFlow.map((step, index) => (
            <div key={step} className="flex items-center gap-2">
              <Link href="/service-operations#status-flow-logs" className="rounded-2xl bg-blue-50 px-3 py-2 text-sm font-bold text-blue-800 ring-1 ring-blue-100 hover:bg-activeBlue hover:text-white">{step}</Link>
      ...
      <SectionCard title="System Health Summary / 系统健康摘要" subtitle="Each module keeps independent health, degraded mode and retry queue. / 每个模块独立健康检查、降级模式和重试队列。">
        <div id="system-health-summary" className="scroll-mt-32 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {moduleHealth.map((item) => (
            <Link key={item.module} href="/system-settings#health-checks" className="rounded-2xl border border-slate-200 bg-white p-4 transition hover:ring-1 hover:ring-activeBlue">
