...
type State = { loading: boolean; error: string | null; result: Record<string, unknown> | null; docs: WarrantyPdf[] };
...
export function ServiceOperationsWarrantyPdfPanel() {
  const [values, setValues] = useState<Values>({ warranty_id: '', visible_to_customer: 'true', generation_notes: 'Warranty PDF generated from completed NANOFIX service warranty.' });
  ...
  return (
    <section className="overflow-hidden rounded-3xl bg-white shadow-soft ring-1 ring-slate-200">
      <div className="bg-slate-50 px-6 py-5">
        ...
        <p className="mt-2 text-sm font-semibold text-slate-600">Generate a warranty certificate PDF from the completed warranty record and document company settings. If visible to customer, Customer Portal can download it. / 根据已生成的保修记录和公司文档设置生成正式保修单 PDF；如设置客户可见，客户门户可下载。</p>
      </div>
      ...
      <div className="grid gap-6 p-6 xl:grid-cols-[0.85fr_1.15fr]">
        <form onSubmit={(event) => void generate(event)} className="grid h-fit gap-4 rounded-3xl bg-slate-50 p-5 ring-1 ring-slate-200">
          ...
          <label className="grid gap-1 text-xs font-black uppercase tracking-[0.08em] text-slate-500">Warranty ID<input className={inputClass} value={values.warranty_id} onChange={(event) => setValues((current) => ({ ...current, warranty_id: event.target.value }))} /></label>
          ...
          <div className="rounded-2xl bg-amber-50 p-4 text-xs font-bold text-amber-950 ring-1 ring-amber-200">PDF is stored privately in Supabase Storage. Customer Portal can download it only if warranty.visible_to_customer=true and the warranty belongs to the customer. / PDF 私有存储；只有保修单客户可见且属于该客户时，客户门户才可下载。</div>
          <div className="flex flex-wrap gap-2">
            <button type="submit" disabled={state.loading} className="rounded-2xl bg-activeBlue px-5 py-3 text-sm font-black text-white hover:bg-blue-700 disabled:opacity-50">Generate Warranty PDF / 生成保修单PDF</button>
