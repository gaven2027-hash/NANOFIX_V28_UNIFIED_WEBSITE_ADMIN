...
const aiIntelligenceMenu: MenuItem = {
  order: '5',
  href: '/ai-intelligence',
  title: 'AI Intelligence Center',
  zh: 'AI 智能中心',
  badge: aiIntelligenceSections.length,
  children: aiIntelligenceSections.map((section) => ({ href: section.href, title: section.title, zh: section.zh }))
};

const customerCenterMenu: MenuItem = {
  order: '6',
  href: '/customer-center',
  title: 'Customer Center',
  zh: '客户相关',
  badge: customerCenterSections.length,
...
const customerPortalMenu: MenuItem = {
  order: 'P1',
  href: '/customer-portal',
  title: 'Customer Portal',
  zh: '客户会员中心入口',
  badge: customerPortalSections.length,
        ...
        return (
          <section
            ...
            className={clsx(
              ...
              active ? 'ring-1 ring-blue-300/50' : 'hover:bg-white/[0.07]'
            )}
          >
            <div className={clsx('flex items-stretch gap-2 p-2 transition', active ? 'bg-activeBlue text-white shadow-lg shadow-blue-950/20' : 'text-slate-100')}>
              <Link href={item.href} onClick={onNavigate} className="group flex min-w-0 flex-1 items-start gap-3 rounded-xl px-2 py-2">
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block text-[13px] font-semibold leading-5 text-slate-200 group-hover:text-white">{item.zh}</span>
                </span>
              ...
              <div className="grid gap-1 px-3 pb-3 pt-2">
                {item.children.map((child) => {
                  ...
                  return (
                    ...
                    >
                      <span className="block text-current">{child.title}</span>
                      <span className={clsx('block text-[12px] font-semibold group-hover:text-slate-100', childActive ? 'text-slate-100' : 'text-slate-300')}>{child.zh}</span>
                    </Link>
                  );
                })}
              </div>
  ...
  return (
    <div className="flex h-20 items-center gap-3 border-b border-white/10 px-6">
      ...
      <div>
        ...
        <div className="text-[13px] font-semibold text-slate-300">V28 / 总后台</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-adminBg text-slate-900">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-80 flex-col bg-sidebar text-white shadow-2xl lg:flex">
        <BrandBlock />
        <SidebarNav />
        <div className="border-t border-white/10 p-4 text-xs font-semibold text-slate-300">QR display is backend-only. Public website QR sections are disabled.</div>
      </aside>
      ...
      <div className="sticky top-0 z-30 flex items-center justify-between bg-sidebar px-4 py-3 text-white shadow-lg lg:hidden">
        <div className="flex items-center gap-3">
          <LogoMark size="sm" />
          <div>
            <div className="text-base font-black text-white">NANOFIX V28</div>
            <div className="text-xs font-semibold text-slate-300">Admin Menu / 后台菜单</div>
          </div>
        </div>
        <button type="button" onClick={() => setMobileMenuOpen((value) => !value)} className="rounded-xl bg-white/10 px-4 py-2 text-sm font-black text-white">
