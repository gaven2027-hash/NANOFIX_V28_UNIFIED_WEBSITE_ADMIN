...
const groups = [
  { key: 'leads', title: 'Leads', zh: '线索', idField: 'lead_id', machine: 'lead', statusField: 'status', nextStatus: 'qualified' },
  { key: 'service_requests', title: 'Service Requests', zh: '报修请求', idField: 'service_request_id', machine: 'service_request', statusField: 'status', nextStatus: 'scheduled' },
  { key: 'jobs', title: 'Jobs', zh: '工单', idField: 'job_id', machine: 'job', statusField: 'status', nextStatus: 'en_route' },
  { key: 'quotations', title: 'Quotations', zh: '报价', idField: 'quotation_id', machine: 'quotation', statusField: 'approval_status', nextStatus: 'approved' },
  { key: 'invoices', title: 'Invoices', zh: '发票', idField: 'invoice_id', machine: 'invoice', statusField: 'status', nextStatus: 'sent' },
  { key: 'payments', title: 'Payments', zh: '付款', idField: 'payment_id', machine: 'payment', statusField: 'status', nextStatus: 'reconciled' },
  { key: 'warranties', title: 'Warranties', zh: '保修', idField: 'warranty_id', machine: 'warranty', statusField: 'status', nextStatus: 'active' }
];
...
export function ServiceOperationsLiveCore() {
  ...
  async function advance(row: CoreRow) {
    ...
    const result = await patchStatus(active.machine, row[active.idField], active.nextStatus);
    if (!result.ok) {
      setAction({ loading: false, message: null, error: result.error ?? 'Status update failed.' });
      return;
    }
    setAction({ loading: false, message: `Status updated to ${active.nextStatus}. Live data refreshed. / 状态已更新并刷新真实数据。`, error: null });
    await loadCore();
  }
  ...
  return (
    <section className="overflow-hidden rounded-3xl bg-white shadow-soft ring-1 ring-slate-200">
      <div className="bg-gradient-to-br from-sky-500 via-cyan-300 to-blue-600 p-6 text-white">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-5xl text-sm font-semibold leading-6 text-white/90">This board reads existing Supabase service-operation tables through a guarded admin API. Status changes use the server RPC `transition_status_tx` and write Audit Logs. / 本面板通过受保护后台 API 读取现有业务主链路表，状态更新走服务端 RPC 并写入审计。</p>
          </div>
      ...
      <div className="p-6">
        ...
        {action.loading || action.message || action.error ? <div className={`mb-5 rounded-3xl p-4 text-xs font-bold ring-1 ${action.error ? 'bg-red-50 text-red-950 ring-red-200' : 'bg-blue-50 text-blue-950 ring-blue-200'}`}>{action.loading ? 'Submitting status update… / 正在提交状态更新…' : action.error ?? action.message}</div> : null}
        ...
        <div className="mt-5 overflow-hidden rounded-3xl ring-1 ring-slate-200">
          ...
          <div className="grid grid-cols-[1fr_110px_110px] gap-3 px-5 py-3 text-xs font-black uppercase tracking-[0.1em] text-slate-500 md:grid-cols-[1.5fr_120px_120px_110px]"><span>Record / 记录</span><span>Status</span><span>Created</span><span className="hidden md:block">Action</span></div>
          {rows.length ? rows.map((row) => (
            <div key={String(row[active.idField])} className="grid grid-cols-[1fr_110px_110px] gap-3 border-t border-slate-200 px-5 py-4 text-sm md:grid-cols-[1.5fr_120px_120px_110px]">
              ...
              <button type="button" disabled={action.loading} onClick={() => void advance(row)} className="hidden rounded-xl bg-activeBlue px-3 py-2 text-xs font-black text-white hover:bg-blue-700 disabled:opacity-50 md:block">Set {active.nextStatus}</button>
            </div>
