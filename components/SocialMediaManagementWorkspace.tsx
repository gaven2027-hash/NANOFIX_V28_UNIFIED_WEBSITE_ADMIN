...
function statusTone(status: unknown): 'blue' | 'green' | 'amber' | 'red' | 'gray' | 'cyan' {
  ...
  if (/(archived|disabled|failed|cancelled|rejected)/i.test(s)) return 'red';
  return 'blue';
}
...
function sectionDefaultJson(section?: SocialMediaSectionConfig | null) {
  ...
  return {
    ...
    notes: 'This social management record is editable from NANOFIX Social Media Management.'
  };
}
...
function Tabs({ active, onChange }: { active: Tab; onChange: (tab: Tab) => void }) {
  const tabs: Array<[Tab, string, string]> = [
    ['records', 'Records & Settings', '记录与设置'],
    ['messages', 'Inbox / Messages', '收件箱/消息'],
    ['drafts', 'AI Drafts & Preview', 'AI 草稿与预览'],
    ['versions', 'Schedule / Publish Versions', '排期/发布版本']
  ];
  return (
...
function RecordEditor({ row, section, onSaved, onClose }: { row: Row | null; section?: SocialMediaSectionConfig | null; onSaved: () => void; onClose: () => void }) {
  ...
  useEffect(() => {
    setForm(row || { section_key: section?.key || 'social-accounts', platform: section?.platform || 'general', title: section?.title || '', body: '', status: 'draft', config_json: sectionDefaultJson(section) });
  ...
  return (
    <div className="rounded-3xl bg-white p-5 shadow-soft ring-1 ring-slate-200">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div><div className="text-xs font-black uppercase tracking-[0.16em] text-activeBlue">{row ? 'Edit Record' : 'New Record'}</div><h3 className="mt-1 text-xl font-black text-slate-950">{row ? formatValue(row.title) : `Create ${section?.title || 'Social Record'}`}</h3></div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <label><span className={labelClass}>Platform / 平台</span><select className={inputClass} value={String(form.platform || 'general')} onChange={(event) => setForm((current) => ({ ...current, platform: event.target.value }))}>{platforms.map((p) => <option key={p} value={p}>{p}</option>)}</select></label>
        ...
        <label className="md:col-span-2"><span className={labelClass}>Body / 内容</span><textarea className={`${inputClass} min-h-24`} value={String(form.body || '')} onChange={(event) => setForm((current) => ({ ...current, body: event.target.value }))} /></label>
...
function DraftEditor({ row, section, onSaved, onClose }: { row: Row | null; section?: SocialMediaSectionConfig | null; onSaved: () => void; onClose: () => void }) {
  ...
  useEffect(() => {
    setForm(row || { section_key: section?.key || 'ai-social-content-studio', platform: section?.platform || 'all', title: section?.title || '', body: '', approval_status: 'draft', prompt_version: section?.key || 'manual', model: 'manual', source_references: [] });
    setSourceReferences(safeJsonString(row?.source_references || []));
    setMessage('');
  }, [row, section]);
  ...
  async function save() {
    ...
    if (parsed === null) {
      setMessage('Source References JSON is invalid. / 来源引用 JSON 格式错误。');
      return;
    }
    setSaving(true);
    ...
    const response = await fetch('/api/admin/social-media', {
      method: row ? 'PATCH' : 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...form, source_references: parsed, action, content_id: row?.content_id })
    });
    const json = await response.json().catch(() => ({}));
    setSaving(false);
    if (!response.ok || !json.ok) {
  ...
  return (
    <div className="rounded-3xl bg-white p-5 shadow-soft ring-1 ring-slate-200">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div><div className="text-xs font-black uppercase tracking-[0.16em] text-activeBlue">{row ? 'Edit Draft' : 'New Draft'}</div><h3 className="mt-1 text-xl font-black text-slate-950">{row ? formatValue(row.title) : `Create ${section?.title || 'Social Draft'}`}</h3></div>
        <button type="button" onClick={onClose} className="rounded-xl bg-slate-100 px-3 py-2 text-xs font-black text-slate-600 hover:bg-slate-200">Close / 关闭</button>
      </div>
      {message ? <div className="mb-4 rounded-2xl bg-blue-50 px-4 py-3 text-sm font-bold text-blue-800 ring-1 ring-blue-100">{message}</div> : null}
      <div className="grid gap-4 md:grid-cols-2">
        <label><span className={labelClass}>Platform / 平台</span><select className={inputClass} value={String(form.platform || 'all')} onChange={(event) => setForm((current) => ({ ...current, platform: event.target.value }))}>{platforms.map((p) => <option key={p} value={p}>{p}</option>)}</select></label>
        <label><span className={labelClass}>Approval Status / 审核状态</span><select className={inputClass} value={String(form.approval_status || 'draft')} onChange={(event) => setForm((current) => ({ ...current, approval_status: event.target.value }))}>{draftStatuses.map((s) => <option key={s} value={s}>{s}</option>)}</select></label>
        <label className="md:col-span-2"><span className={labelClass}>Title / 标题</span><input className={inputClass} value={String(form.title || '')} onChange={(event) => setForm((current) => ({ ...current, title: event.target.value }))} /></label>
        <label className="md:col-span-2"><span className={labelClass}>Post Body / 文案</span><textarea className={`${inputClass} min-h-40`} value={String(form.body || '')} onChange={(event) => setForm((current) => ({ ...current, body: event.target.value }))} /></label>
        <label><span className={labelClass}>Prompt Version / 提示词版本</span><input className={inputClass} value={String(form.prompt_version || '')} onChange={(event) => setForm((current) => ({ ...current, prompt_version: event.target.value }))} /></label>
        <label><span className={labelClass}>Model / 模型</span><input className={inputClass} value={String(form.model || 'manual')} onChange={(event) => setForm((current) => ({ ...current, model: event.target.value }))} /></label>
        <label className="md:col-span-2"><span className={labelClass}>Source References JSON / 来源引用 JSON</span><textarea className={`${inputClass} min-h-28 font-mono text-xs`} value={sourceReferences} onChange={(event) => setSourceReferences(event.target.value)} /></label>
      </div>
      <button type="button" disabled={saving} onClick={save} className="mt-4 rounded-2xl bg-activeBlue px-5 py-3 text-sm font-black text-white shadow-sm hover:bg-blue-700 disabled:opacity-60">Save Draft / 保存草稿</button>
    </div>
  );
}
...
export function SocialMediaManagementWorkspace({ section }: { section?: SocialMediaSectionConfig | null }) {
  ...
  useEffect(() => {
    ...
    setPlatform(section?.platform || 'all');
  ...
  async function publishSnapshot(row?: Row | null) {
    ...
    const response = await fetch('/api/admin/social-media', {
      ...
      body: JSON.stringify({
        ...
        platform,
        status: 'scheduled',
        snapshot_json: { section_key: section?.key, platform, source, created_at: new Date().toISOString(), ai_auto_publish_allowed: false }
      })
    });
    const json = await response.json().catch(() => ({}));
    if (!response.ok || !json.ok) {
      setMessage(json.error || 'Publish snapshot failed. / 发布快照失败。');
      return;
    }
    setMessage(`Publish snapshot saved as version ${json.version?.version_no}. / 已保存发布版本 ${json.version?.version_no}。`);
    await loadAll();
  }
    ...
    <div>
      ...
      {section ? (
        <div className="mb-5 rounded-3xl bg-white p-5 shadow-soft ring-1 ring-slate-200">
          <div className="text-xs font-black uppercase tracking-[0.16em] text-activeBlue">Social Media Section / 社媒管理二级栏目</div>
          ...
          <div className="mt-3 flex flex-wrap gap-2"><Badge tone="blue">Platform: {section.platform}</Badge><Badge tone="amber">AI no auto publish</Badge><Badge tone="green">Audit log enabled</Badge></div>
        </div>
      ) : null}
      <Tabs active={activeTab} onChange={setActiveTab} />
      {message ? <div className="mb-4 rounded-2xl bg-blue-50 px-4 py-3 text-sm font-bold text-blue-800 ring-1 ring-blue-100">{message}</div> : null}

      <SectionCard title="Social Media Control Center / 社媒控制中心" subtitle="Manage social records, inbox messages, AI drafts, platform previews, scheduling and publish versions. / 管理社媒记录、消息、AI 草稿、平台预览、排期和发布版本。">
        <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_220px_220px_auto_auto]">
          <input className={inputClass} placeholder="Search title, body, channel, platform... / 搜索" value={search} onChange={(event) => setSearch(event.target.value)} />
          <select className={inputClass} value={platform} onChange={(event) => setPlatform(event.target.value)}>{platforms.map((p) => <option key={p} value={p}>{p}</option>)}</select>
          ...
          <button type="button" onClick={() => publishSnapshot()} className="rounded-2xl bg-activeBlue px-5 py-2 text-sm font-black text-white hover:bg-blue-700">Save Publish Snapshot / 保存发布版本</button>
        </div>
      </SectionCard>

      {activeTab === 'records' ? (
        <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_420px]">
          <SectionCard title="Records & Settings / 记录与设置" subtitle="Manage account settings, response rules, review workflows and platform configuration. / 管理账号、回复规则、审核流程和平台配置。">
            ...
            <div className="overflow-x-auto rounded-2xl border border-slate-200"><table className="min-w-[880px] w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="p-3">Status</th><th className="p-3">Section</th><th className="p-3">Platform</th><th className="p-3">Title</th><th className="p-3">Updated</th><th className="p-3">Action</th></tr></thead><tbody className="divide-y divide-slate-100">
              ...
              {!records.length ? <tr><td colSpan={6} className="p-6 text-center text-sm font-bold text-slate-500">{loading ? 'Loading...' : 'No records yet. / 暂无记录。'}</td></tr> : null}
            </tbody></table></div>
          </SectionCard>
      ...
      {activeTab === 'messages' ? (
        <div className="mt-5"><SectionCard title="Inbox / Messages / 收件箱与消息" subtitle="Search and review real social_messages records. / 搜索和查看真实 social_messages 记录。">
          <div className="overflow-x-auto rounded-2xl border border-slate-200"><table className="min-w-[960px] w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="p-3">Channel</th><th className="p-3">Direction</th><th className="p-3">Risk</th><th className="p-3">Body</th><th className="p-3">Created</th></tr></thead><tbody className="divide-y divide-slate-100">
            {messages.map((row) => <tr key={String(row.message_id)} className="hover:bg-blue-50/50"><td className="p-3 font-bold text-slate-800">{formatValue(row.channel)}</td><td className="p-3 text-slate-600">{formatValue(row.direction)}</td><td className="p-3"><Badge tone={String(row.risk_level).includes('high') ? 'red' : 'blue'}>{formatValue(row.risk_level)}</Badge></td><td className="max-w-xl truncate p-3 font-semibold text-slate-700">{formatValue(row.body)}</td><td className="p-3 text-slate-500">{formatValue(row.created_at)}</td></tr>)}
            {!messages.length ? <tr><td colSpan={5} className="p-6 text-center text-sm font-bold text-slate-500">{loading ? 'Loading...' : 'No messages yet. / 暂无消息。'}</td></tr> : null}
          </tbody></table></div>
        </SectionCard></div>
      ) : null}
      ...
      {activeTab === 'drafts' ? (
        <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_460px]">
          <SectionCard title="AI Drafts & Platform Preview / AI 草稿与平台预览" subtitle="Create and edit platform-specific drafts. Admin approval is required before scheduling or publishing. / 创建和编辑平台独立草稿，发布前必须人工审核。">
            ...
            <div className="overflow-x-auto rounded-2xl border border-slate-200"><table className="min-w-[920px] w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="p-3">Status</th><th className="p-3">Platform</th><th className="p-3">Title</th><th className="p-3">Prompt</th><th className="p-3">Created</th><th className="p-3">Action</th></tr></thead><tbody className="divide-y divide-slate-100">
              {drafts.map((draft) => <tr key={String(draft.content_id)} className="hover:bg-blue-50/50"><td className="p-3"><Badge tone={statusTone(draft.approval_status)}>{formatValue(draft.approval_status)}</Badge></td><td className="p-3 font-bold text-slate-800">{formatValue(draft.platform)}</td><td className="max-w-72 truncate p-3 font-semibold text-slate-700">{formatValue(draft.title)}</td><td className="p-3 text-slate-600">{formatValue(draft.prompt_version)}</td><td className="p-3 text-slate-500">{formatValue(draft.created_at)}</td><td className="p-3"><button type="button" onClick={() => { setSelectedDraft(draft); setCreatingDraft(false); }} className="rounded-xl bg-white px-3 py-2 text-xs font-black text-activeBlue ring-1 ring-blue-100 hover:bg-blue-50">Open / 打开</button></td></tr>)}
              {!drafts.length ? <tr><td colSpan={6} className="p-6 text-center text-sm font-bold text-slate-500">{loading ? 'Loading...' : 'No drafts yet. / 暂无草稿。'}</td></tr> : null}
            </tbody></table></div>
          </SectionCard>
      ...
      {activeTab === 'versions' ? (
        <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px]">
          <SectionCard title="Schedule / Publish Versions / 排期与发布版本" subtitle="Store approval/schedule/publish snapshots for audit and rollback. / 保存审批、排期、发布快照，便于审计和回溯。">
            <div className="overflow-x-auto rounded-2xl border border-slate-200"><table className="min-w-[860px] w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="p-3">Status</th><th className="p-3">Platform</th><th className="p-3">Version</th><th className="p-3">Scheduled</th><th className="p-3">Created</th></tr></thead><tbody className="divide-y divide-slate-100">
              {versions.map((version) => <tr key={String(version.version_id)}><td className="p-3"><Badge tone={statusTone(version.status)}>{formatValue(version.status)}</Badge></td><td className="p-3 font-bold text-slate-800">{formatValue(version.platform)}</td><td className="p-3"><Badge tone="green">v{formatValue(version.version_no)}</Badge></td><td className="p-3 text-slate-600">{formatValue(version.scheduled_at)}</td><td className="p-3 text-slate-500">{formatValue(version.created_at)}</td></tr>)}
              {!versions.length ? <tr><td colSpan={5} className="p-6 text-center text-sm font-bold text-slate-500">No publish versions yet. / 暂无发布版本。</td></tr> : null}
            </tbody></table></div>
          </SectionCard>
          <section className="rounded-3xl bg-white p-5 shadow-soft ring-1 ring-slate-200"><h3 className="text-lg font-black text-slate-950">AI Publish Safety / AI 发布安全</h3><p className="mt-3 text-sm leading-7 text-slate-600">AI-generated social content is always saved as draft first. Admin can approve, schedule or create publish snapshots, but AI cannot auto-publish by default.</p><button type="button" onClick={() => publishSnapshot()} className="mt-4 rounded-2xl bg-activeBlue px-4 py-3 text-sm font-black text-white hover:bg-blue-700">Create Snapshot / 创建快照</button></section>
        </div>
      ) : null}
    </div>
  );
