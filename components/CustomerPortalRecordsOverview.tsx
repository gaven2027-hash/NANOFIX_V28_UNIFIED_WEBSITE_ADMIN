...
const sections: Array<{ key: keyof Payload; title: string; zh: string; empty: string; fields: string[] }> = [
  { key: 'service_requests', title: 'Repair Requests', zh: '报修记录', empty: 'No repair requests yet. / 暂无报修记录。', fields: ['status', 'leak_location', 'issue_description', 'address_text', 'created_at'] },
  { key: 'jobs', title: 'Jobs & Site Works', zh: '工单与施工', empty: 'No jobs yet. / 暂无工单。', fields: ['status', 'scheduled_at', 'notes', 'created_at'] },
  { key: 'invoices', title: 'Invoices', zh: '发票', empty: 'No invoices yet. / 暂无发票。', fields: ['invoice_no', 'total', 'status', 'created_at'] },
  { key: 'payments', title: 'Payments', zh: '付款', empty: 'No payments yet. / 暂无付款记录。', fields: ['amount', 'status', 'fee', 'reconciled_at', 'created_at'] },
  { key: 'warranties', title: 'Warranties', zh: '保修', empty: 'No warranties yet. / 暂无保修记录。', fields: ['status', 'coverage', 'starts_at', 'ends_at', 'created_at'] }
];
...
function Section({ title, zh, empty, rows, fields }: { title: string; zh: string; empty: string; rows: Row[]; fields: string[] }) {
  return (
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-6 grid gap-5">
          {sections.map((section) => {
            ...
            return <Section key={section.key} title={section.title} zh={section.zh} empty={section.empty} rows={rows} fields={section.fields} />;
          })}
        </div>
      </div>
    </main>
