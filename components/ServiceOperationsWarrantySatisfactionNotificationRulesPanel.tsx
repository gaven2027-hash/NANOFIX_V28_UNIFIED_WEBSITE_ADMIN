  ...
  return (
    <section id="warranty-satisfaction-notification-rules" className="scroll-mt-32 rounded-3xl bg-white p-5 shadow-soft ring-1 ring-slate-200">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="text-xs font-black uppercase tracking-[0.16em] text-activeBlue">Warranty Satisfaction Notification Rules</div>
          <h3 className="mt-2 text-xl font-black text-slate-950">Warranty Satisfaction Notification Rules / 保修满意度通知规则</h3>
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        <button type="button" onClick={() => void submit(false)} disabled={state.saving} className="rounded-2xl bg-slate-100 px-4 py-3 text-sm font-black text-slate-800 hover:bg-slate-200 disabled:opacity-50">Preview Rules / 预览规则</button>
        <button type="button" onClick={() => void submit(true)} disabled={state.saving} className="rounded-2xl bg-activeBlue px-4 py-3 text-sm font-black text-white hover:bg-blue-700 disabled:opacity-50">Apply Rules / 执行规则</button>
      </div>
    </section>
  );
}
