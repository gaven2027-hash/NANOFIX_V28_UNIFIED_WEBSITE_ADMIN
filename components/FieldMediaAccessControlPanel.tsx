...
function RoleBox({ title, roles, selected, onChange }: { title: string; roles: string[]; selected: string[]; onChange: (value: string[]) => void }) {
  return <div className="rounded-2xl bg-slate-50 p-3 ring-1 ring-slate-200"><span className={labelClass}>{title}</span><div className="grid gap-2 md:grid-cols-2">{orderedRoles(roles).map((role) => <label key={role} className="flex items-center gap-2 rounded-xl bg-white px-3 py-2 text-xs font-bold text-slate-700 ring-1 ring-slate-100"><input type="checkbox" checked={selected.includes(role)} onChange={() => onChange(toggle(selected, role))} />{roleLabels[role] || role}</label>)}</div></div>;
}
