...
const lanes: Lane[] = [
  { id: 'automation-notification-engine', title: 'Automation & Notification Engine', zh: '自动化与通知引擎', metric: 'Rules → Outbox → Audit', note: 'Trigger module events and queue internal notifications through live Supabase-backed workflow records.' },
...
export function AutomationNotificationWorkspace() {
  ...
  async function acknowledgeInboxMessage(message: InboxApiMessage) {
    if (!isUuid(message.message_id)) {
      setActionState({ loading: false, message: null, error: 'A live inbox message_id is required before acknowledging. / 需要真实 message_id 才能确认。' });
      return;
    }
  ...
  async function advanceTask(task: UnifiedTask) {
    if (!isUuid(task.task_id)) {
      setActionState({ loading: false, message: null, error: 'A live task_id is required before updating. / 需要真实 task_id 才能更新。' });
      return;
    }
    ...
    <section id="automation-notification-engine" className="mt-6 scroll-mt-40 overflow-hidden rounded-3xl bg-white shadow-soft ring-1 ring-slate-200">
      ...
      <div className="p-6">
        <div className={`mb-5 rounded-3xl p-4 ring-1 ${live.degraded ? 'bg-amber-50 text-amber-950 ring-amber-200' : 'bg-emerald-50 text-emerald-950 ring-emerald-200'}`}>
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs font-semibold leading-5">{live.degraded ? 'Some APIs are not available. This usually means no login session, missing Supabase migration, or database readiness failure. No local records are shown as live data. / 部分 API 不可用，通常是未登录、未执行 Supabase migration 或数据库未 ready；本地记录不会显示为真实数据。' : 'Automation, inbox and task APIs responded successfully. / 自动化、内部收件箱和任务 API 已成功响应。'}</p>
            </div>
        ...
        <div className={`mb-5 rounded-3xl p-4 ring-1 ${actionState.error ? 'bg-red-50 text-red-950 ring-red-200' : actionState.message ? 'bg-blue-50 text-blue-950 ring-blue-200' : 'bg-slate-50 text-slate-700 ring-slate-200'}`}>
          ...
          <p className="mt-1 text-xs font-semibold leading-5">Buttons below call server APIs, write Audit Logs through the API layer, then refresh live data after a successful API response. / 下方按钮调用服务端 API，经 API 层写入审计日志，并在 API 成功后刷新真实数据。</p>
