...
const actions: Array<{ key: ActionKind; title: string; zh: string; note: string }> = [
  ...
  { key: 'submit_inspection_form', title: 'Inspection Form', zh: '查验表单', note: 'Submit findings, diagnosis and recommended action.' },
  { key: 'assign_engineer', title: 'Engineer Assignment', zh: '工程师分配', note: 'Assign an engineer to an inspection. Manager roles only.' },
  { key: 'create_upload_review', title: 'Create Upload Review', zh: '创建上传审核', note: 'Register uploaded photo/video/document metadata for review.' },
  { key: 'review_upload', title: 'Review Upload', zh: '审核上传', note: 'Approve, reject or mark upload for redaction.' }
];
...
const defaults: Record<ActionKind, Values> = {
  ...
  create_upload_review: { inspection_id: '', service_request_id: '', job_id: '', file_name: '', file_type: 'image', storage_path: '', review_notes: '' },
...
export function ServiceOperationsInspectionWorkspace() {
  const [active, setActive] = useState<ActionKind>('schedule_inspection');
  ...
  async function load() {
    ...
    setState({
      ...
      inspections: Array.isArray(payload?.inspections) ? payload.inspections as Row[] : [],
      upload_reviews: Array.isArray(payload?.upload_reviews) ? payload.upload_reviews as Row[] : [],
  ...
  return (
    <section className="overflow-hidden rounded-3xl bg-white shadow-soft ring-1 ring-slate-200">
      <div className="bg-slate-50 px-6 py-5">
        ...
        <p className="mt-2 text-sm font-semibold text-slate-600">This workspace calls `/api/admin/service-operations/inspections`, writes `service_inspections` and `service_upload_reviews`, and records Audit Logs. / 本工作区调用真实 API，写入查验与上传审核表，并记录审计日志。</p>
      </div>
      <div className="p-6">
